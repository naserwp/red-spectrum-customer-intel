...
export const config = {
  matcher: ["/admin/:path*", "/api/woocommerce/backfill-orders", "/api/woocommerce/backfill-subscriptions", "/api/authorize-net/backfill-transactions", "/api/authorize-net/reconcile-customers", "/api/nmi/backfill-transactions", "/api/nmi/reconcile-customers", "/api/nmi/repair-customer", "/api/wordpress/debug-user-meta", "/api/sync/run-step", "/api/customers/rebuild-from-orders", "/api/customers/sync-one", "/api/customers/compare-source"],
};
